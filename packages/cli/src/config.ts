...
// The shape of the config file. Using `Record<string, string>` (rather than
// a strict interface with fixed keys) on purpose — it lets us add new
// config keys over time (e.g. "remoteUrl") without having to update this
// type and every place that reads it. Validation of specific keys, if ever
// needed, can happen at the call site.
export type SendkitConfig = Record<string, string>;
 ...
 */
export function readConfig(): SendkitConfig {
  ...
  try {
    ...
    if (
      ...
      console.error(
        `Warning: ${CONFIG_PATH} does not contain a valid config object. Ignoring it.`,
      );
      return {};
    }

    return parsed as SendkitConfig;
  } catch (err) {
    console.error(
      `Warning: could not parse ${CONFIG_PATH} (${(err as Error).message}). Ignoring it.`,
    );
    return {};
  }
}
 ...
 */
function writeConfig(config: SendkitConfig): void {
 ...
 */
export function listConfig(): SendkitConfig {
  return readConfig();
}
